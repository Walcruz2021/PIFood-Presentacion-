...
export default function DetailsRecipe(props) {
  ...
  return (
    <main class="container">
      <h3 class="centrar-texto">DETAILS RECIPE</h3>
      <div>
        {Alldetails.length > 0 ? (
          <div>
            <div class="grid">
              ...
              <div class="column-7">
                <h5 class="title2">NAME OF RECIPE:</h5>
                {Alldetails[0].name}
                <br></br>
                {Alldetails[0].score ? (
                  <p class="title2">SCORE: {Alldetails[0].score}</p>
                ) : (
                ...
                {Alldetails[0].health_Score ? (
                  <p class="title2">
                    HEALTH SCORE: {Alldetails[0].health_Score}
                  </p>
                ) : (
                ...
                {Alldetails[0].TypeDiets &&
                Alldetails[0].TypeDiets.length > 0 ? (
                ...
                {Alldetails[0].dishTypes &&
                Alldetails[0].dishTypes.length > 0 ? (
            ...
            <p class="sumary">{Alldetails[0].summary}</p>

            <p class="title">STEPS BY STEPS:</p>
            {Alldetails[0].createinDB
              ? (auxcadena = dividirCadena(
                  Alldetails[0].stepsBySteps,
                  punto
                )).map((step, i) => {
                  return (
                    <div className="step">
              ...
              : Alldetails[0].stepsBySteps &&
                Alldetails[0].stepsBySteps.length > 0 && (
                  <div>
                    {Alldetails[0].stepsBySteps.map((step, i) => {
                      return (
                        <div className="step">
                          {Alldetails[0].stepsBySteps.length > 1 && (
                            <span>{i + 1}. </span>
                          )}
                          <span>{step}</span>
                        </div>
      ...
      <Link to="/home">
        <button class="btn btn-outline-success btn-block">Back to Home</button>
      </Link>
    </main>
  );
}
